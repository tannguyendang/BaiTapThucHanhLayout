...
export default class BaiTapLayoutMap extends Component {
    ...
    renderProduct = () => {
        const contentProduct = this.productList.map((product, index) => {
            return <div className="col-4 mt-5" key={index}>
                <div className="card text-left">
                    <div style={{ background: `url('${product.hinhAnh}') center`, backgroundSize: 'cover' }}>
                        <img style={{ opacity: '0' }} className="card-img-top" src={product.hinhAnh} height={300} alt={product.tenPhim} />
                    </div>
                    <div className="card-body">
                        <h4 style={{ height: 50 }} className="card-title">{product.tenPhim}</h4>
    ...
    render() {
        return (
            <div className="container">
                <h1 className="display-4">Danh sách phim</h1>
                <div className="row">
                    {this.renderProduct()}
                </div>
            </div>
